...
def train_policy(env: gym.Env, start_node, end_node, n_episodes=3000, minimum_reach=0.9):
    ...
    model = PPO("MlpPolicy", env, verbose=0,  n_steps = 4096, batch_size = 128)
...
def test_policy(policy: PPO, env: gym.Env, start_node, end_node, n_episodes=3000):
    ...
    for episode in episodes:
        ...
        episodes.set_description(f"Current reach: {sum(reach[-1000:])/1000:.2f}, total_reach: {sum(reach)}, reward: {statistics.mean(rewards):.2f}±{statistics.stdev(rewards):.1f}")
